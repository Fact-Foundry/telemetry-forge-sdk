...
/// <summary>
/// Resolves country and region from CDN/reverse proxy geolocation headers on the
/// inbound request. API telemetry carries no caller IP, so the caller's country can
/// only be known from these edge-injected headers — not from the connection the SDK
/// uses to post telemetry.
/// </summary>
